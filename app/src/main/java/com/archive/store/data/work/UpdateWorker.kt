package com.archive.store.data.work
...
import com.archive.store.data.providers.GoogleAccountTokenProvider
...
import com.archive.store.util.Preferences.PREFERENCE_SELF_UPDATE_ENABLED
 ...
 *
 * Archive Store's own update is fetched from the bundled release/nightly feed and added
...
class UpdateWorker @AssistedInject constructor(
    ...
    private val isArchiveOnlyFilterEnabled: Boolean
        get() = Preferences.getBoolean(context, Preferences.PREFERENCE_FILTER_ARCHIVE_ONLY, false)
    ...
    override suspend fun doWork(): Result {
        ...
        try {
            ...
            // Clean the update list to prepare for installing. Archive Store's own update
    ...
    private suspend fun checkUpdates(): List<Update> {
        return withContext(Dispatchers.IO) {
            ...
            val filteredPackages = if (isArchiveOnlyFilterEnabled) {
                packages.filter { CertUtil.isArchiveStoreApp(context, it.packageName) }
            } else {
            ...
            // Archive Store's own update comes from the feed, not Play. When one is
