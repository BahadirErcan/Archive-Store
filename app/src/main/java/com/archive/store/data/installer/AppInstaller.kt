...
package com.archive.store.data.installer
...
import com.archive.extensions.getUpdateOwnerPackageNameCompat
import com.archive.extensions.isOAndAbove
import com.archive.extensions.isPAndAbove
...
import com.archive.store.data.installer.ShizukuInstaller.Companion.SHIZUKU_PACKAGE_NAME
import com.archive.store.data.installer.base.IInstaller
...
import com.archive.store.util.PackageUtil.hasMicroGCompanion
...
class AppInstaller @Inject constructor(
    ...
    companion object {
        const val ACTION_INSTALL_STATUS =
            "com.archive.store.data.installer.AppInstaller.INSTALL_STATUS"

        const val EXTRA_PACKAGE_NAME =
            "com.archive.store.data.installer.AppInstaller.EXTRA_PACKAGE_NAME"
        const val EXTRA_VERSION_CODE =
            "com.archive.store.data.installer.AppInstaller.EXTRA_VERSION_CODE"
        const val EXTRA_DISPLAY_NAME =
            "com.archive.store.data.installer.AppInstaller.EXTRA_DISPLAY_NAME"
        ...
        fun getAvailableInstallersInfo(context: Context): List<InstallerInfo> = listOfNotNull(
            ...
            if (hasArchiveService(context)) ServiceInstaller.installerInfo else null,
        ...
        fun canInstallSilently(context: Context, packageName: String, targetSdk: Int): Boolean {
            return when (getCurrentInstaller(context)) {
                ...
                Installer.SERVICE -> hasArchiveService(context)
        ...
        fun hasArchiveService(context: Context): Boolean = try {
    ...
    fun getPreferredInstaller(notifyOnFallback: Boolean = false): IInstaller {
        ...
        val installer = when (selected) {
            ...
            Installer.SERVICE -> if (hasArchiveService(context)) {
                serviceInstaller
            } else {
                defaultInstaller
            }
