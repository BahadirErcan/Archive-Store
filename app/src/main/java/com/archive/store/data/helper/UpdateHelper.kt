package com.archive.store.data.helper
...
import com.archive.store.data.work.UpdateWorker
import com.archive.store.util.PackageUtil
import com.archive.store.util.Preferences
import com.archive.store.util.Preferences.PREFERENCES_UPDATES_RESTRICTIONS_BATTERY
import com.archive.store.util.Preferences.PREFERENCES_UPDATES_RESTRICTIONS_IDLE
import com.archive.store.util.Preferences.PREFERENCES_UPDATES_RESTRICTIONS_METERED
...
class UpdateHelper @Inject constructor(
    ...
    }.stateIn(ArchiveApp.scope, SharingStarted.WhileSubscribed(), null)
    ...
    }.stateIn(ArchiveApp.scope, SharingStarted.WhileSubscribed(), emptyList())
    ...
    val isCheckingUpdates = WorkManager.getInstance(context)
        ...
        .stateIn(ArchiveApp.scope, SharingStarted.WhileSubscribed(), false)

    /**
     * Deletes invalid updates from database and starts observing events
     */
    fun init() {
        ArchiveApp.scope.launch {
    ...
    private fun observeUpdates() {
        ...
        ArchiveApp.events.busEvent.onEach {
    ...
    /**
     * Immediately drops Archive Store's own update row, e.g. when the user turns the
     * self-update preference off. The periodic check won't re-add it while disabled.
     */
    fun deleteSelfUpdate() {
        ArchiveApp.scope.launch { deleteUpdate(BuildConfig.APPLICATION_ID) }
    }
