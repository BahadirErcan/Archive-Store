...
import com.archive.services.IPrivilegedCallback
import com.archive.services.IPrivilegedService
...
class ServiceInstaller @Inject constructor(
    ...
    companion object {
        ...
        const val PRIVILEGED_EXTENSION_PACKAGE_NAME = "com.archive.services"
        const val PRIVILEGED_EXTENSION_SERVICE_INTENT = "com.archive.services.IPrivilegedService"
            ...
            Handler(Looper.getMainLooper()).post {
                serviceConnection = object : ServiceConnection {
                    override fun onServiceConnected(name: ComponentName, binder: IBinder) {
                        ...
                        ArchiveApp.enqueuedInstalls.add(packageName)
                    ...
                    override fun onServiceDisconnected(name: ComponentName) {
                        removeFromInstallQueue(packageName)
                        readyWithAction.set(true)
                        Log.e(TAG, "Disconnected from Archive Services")
                    }
                }
        ...
        try {
            when (returnCode) {
                PackageInstaller.STATUS_SUCCESS -> {
                    ArchiveApp.events.send(InstallerEvent.Uninstalled(packageName = packageName))
                }
    ...
    private fun handleCallback(packageName: String, returnCode: Int, extra: String?) {
        ...
        try {
            when (returnCode) {
                PackageInstaller.STATUS_SUCCESS -> {
                    ArchiveApp.events.send(InstallerEvent.Installed(packageName = packageName))
