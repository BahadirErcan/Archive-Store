...
import com.archive.store.compose.ui.sheets.DeepLinkConfirmSheet
 ...
 * (market:// and play.google.com links). These are the vector ads exploit to launch Archive into a
...
class DeepLinkConfirmActivity : FragmentActivity() {
        ...
        setContent {
            ArchiveTheme {
                DeepLinkConfirmSheet(
                    ...
                    onOpen = { forwardToArchive(target) },
                    onDismiss = { finish() }
                )
            }
        }
    ...
    private fun forwardToArchive(target: Screen?) {
        startActivity(
            Intent(this, ComposeActivity::class.java).apply {
                target?.let { putExtra(Screen.PARCEL_KEY, it) }
                // Start ComposeActivity fresh so the parcel is honoured even when Archive is already
